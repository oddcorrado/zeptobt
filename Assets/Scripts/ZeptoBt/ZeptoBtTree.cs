...
public class ZeptoBtTree : MonoBehaviour
{
    ...
    private NodeRoot Root { get; set; } = new NodeRoot();
    ...
    static public Dictionary<string, string> StringToAction { get; set; } = new Dictionary<string, string>()
    {
        {"root", "ZeptoBt.NodeRoot" },
        {">", "ZeptoBt.NodeSequence" },
        {"?", "ZeptoBt.NodeSelector" },
        {"wait", "ZeptoBt.NodeLeafWait" },
        {"vel", "ZeptoBt.NodeLeafVelocity" },
        {"trigger", "ZeptoBt.NodeLeafTrigger" },
        {"$", "ZeptoBt.NodeLeafExpression" },
        {"scale", "ZeptoBt.NodeLeafScale" },
        {"moveto", "ZeptoBt.NodeLeafMoveTo" },
        {"roam", "ZeptoBt.NodeLeafRoam" },
        {"prowl", "ZeptoBt.NodeLeafProwl" },
        {"dodge", "ZeptoBt.NodeLeafDodge" },
        {"spine" , "ZeptoBt.NodeLeafSpine"},
        {"hit", "ZeptoBt.NodeLeafHit" },
        {"activate", "ZeptoBt.NodeLeafActivate"},
        {"invertify", "ZeptoBt.NodeDecoratorInvert"},
        {"successify", "ZeptoBt.NodeDecoratorSuccessify"},
        {"failify", "ZeptoBt.NodeDecoratorForceFailify"},
        {"onceify", "ZeptoBt.NodeDecoratorOnce"}
    };

    public void ReadData()
    {
        if (Fili.FileExists($"monsters/{filename}"))
        {
            Debug.Log($" READING CSV ./Data/monsters/{filename}");
            FileData = Fili.ReadAllText($"monsters/{filename}");
        }
    }
    ...
    public void CreateTree()
    {
        ...
        lines.ForEach(line =>
        {
            ...
            switch (datas[0])
            {
                ...
                default:
                    var className = StringToAction[datas[0]];
    ...
    private IEnumerator Start()
    {
        ...
        // FIXME
        FindObjectOfType<ZeptoBtViewer>().Root = Root;
    }
